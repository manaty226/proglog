...
import (
	...
	"github.com/manaty226/proglog/internal/log"
...
func testConsumePastBoundary(
	...
	if got != want {
		t.Fatalf("got err: %v, but want %v", got, want)
	}
}
...
func testUnauthorized(
	...
	if produce != nil {
		t.Fatalf("produce response should be nil, but got: %v", produce)
	}
	...
	if consume != nil {
		t.Fatalf("consume response should be nil, but got: %v", consume)
	}
	gotCode, wantCode = status.Code(err), codes.PermissionDenied
	if gotCode != wantCode {
		t.Fatalf("got code: %d, but want: %d", gotCode, wantCode)
